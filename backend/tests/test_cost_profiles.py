...
def test_roundtrip(store):
    profile = {"spread": 0.8, "slippage": {"kind": "fixed", "value": 0.1, "atrMult": 0},
               ...
               "source": "broker"}
    asyncio.run(store.upsert("dev", "US100", profile))
    got = asyncio.run(store.get("dev", "US100"))
...
def test_missing_epic_is_none(store):
    assert asyncio.run(store.get("dev", "NOPE")) is None


def test_upsert_overwrites(store):
    asyncio.run(store.upsert("dev", "EURUSD", {"spread": 1.0, "source": "broker"}))
    asyncio.run(store.upsert("dev", "EURUSD", {"spread": 2.0, "source": "manual"}))
    got = asyncio.run(store.get("dev", "EURUSD"))
...
def test_corrupt_slippage_json_returns_defaults(store):
    ...
    import sqlite3
    asyncio.run(store.upsert("dev", "BAD", {"spread": 1.0, "source": "manual"}))
    with sqlite3.connect(store.db_path) as con:
        con.execute(
            "UPDATE cost_profiles SET slippage_json='{oops' "
            "WHERE user_id='dev' AND epic='BAD'"
        )
    got = asyncio.run(store.get("dev", "BAD"))
