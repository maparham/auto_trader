...
# cache serves it.
BROKER_HEALTH = BrokerHealth(per_key_timeout={"mt5": 90.0, "mt5-self": 60.0, "dukascopy": 45.0, "oanor": 20.0, "nobitex": 30.0})
...
async def _fetch_symbol_candles(
    ...
    `budget_s` / `partial` (see CandleCache.window): a fill time budget and the
