...
export function computePivotBands(
  ...
  if (mtf?.timeframe && mtf.htfStarts && mtf.htfHigh && mtf.htfLow && mtf.htfMs) {
    ...
    const high = alignHtfToChart(ts, htfBars, mtf.htfHigh, mtf.htfMs, true, mtf.formingIdx, mtf.chartMs, mtf.timeframe);
    const low = alignHtfToChart(ts, htfBars, mtf.htfLow, mtf.htfMs, true, mtf.formingIdx, mtf.chartMs, mtf.timeframe);
