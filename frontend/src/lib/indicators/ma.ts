...
export function computeMa(
  ...
  if (mtf?.timeframe && mtf.htfSeries && mtf.htfStarts && mtf.htfMs) {
    ...
    const aligned = alignHtfToChart(
      chartTs, htfBars, mtf.htfSeries, mtf.htfMs, true, mtf.formingIdx, mtf.chartMs, mtf.timeframe,
    );
    const smoothed = mtf.htfSmoothing
      ? alignHtfToChart(
          chartTs, htfBars, mtf.htfSmoothing, mtf.htfMs, true, mtf.formingIdx, mtf.chartMs, mtf.timeframe,
        )
      : undefined;
    ...
    const alignedStates = ext.slopeColor?.enabled
      ? alignHtfToChart(
          ...
          mtf.htfMs, true, mtf.formingIdx, mtf.chartMs, mtf.timeframe,
        )
      : undefined;
