...
export function computePivotBarsSince(
  ...
  if (mtf?.timeframe && mtf.htfStarts && mtf.htfBarsSinceHigh && mtf.htfBarsSinceLow && mtf.htfMs) {
    ...
    const high = alignHtfToChart(ts, htfBars, mtf.htfBarsSinceHigh, mtf.htfMs, true, mtf.formingIdx, mtf.chartMs, mtf.timeframe);
    const low = alignHtfToChart(ts, htfBars, mtf.htfBarsSinceLow, mtf.htfMs, true, mtf.formingIdx, mtf.chartMs, mtf.timeframe);
